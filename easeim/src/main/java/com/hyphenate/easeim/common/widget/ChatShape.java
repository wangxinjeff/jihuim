...
public class ChatShape extends Shape {
    ...
    private void resizePath(float width, float height, Path path) {
        ...
        /*绘制左上方圆角*/
        connerRect.set(widthStart + arrowWidth, heightStart, widthStart + arrowWidth + connerRadius, heightStart + connerRadius);
        path.arcTo(connerRect, 180, 90);
        /*上方直线边框*/
        path.lineTo(width-connerRadius, heightStart);
        /*右上方圆角*/
        connerRect.set(width - connerRadius, heightStart, width, heightStart + connerRadius);
