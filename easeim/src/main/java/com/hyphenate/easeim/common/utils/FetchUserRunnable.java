...
public class FetchUserRunnable implements Runnable{
    ...
    private void warpEMUserInfo(Map<String, EMUserInfo> userInfos){
        ...
        Map<String, EaseUser> exitUsers = EaseIMHelper.getInstance().getContactList();
        while(it_user.hasNext()) {
            ...
            if (userInfo != null) {
                ...
                LiveDataBus.get().with(EaseCallKitUtils.UPDATE_USERINFO).postValue(info);
            }
        }

        //更新本地数据库信息
        EaseIMHelper.getInstance().updateUserList(userEntities);
        ...
        if(refreshContact) {
            //通知UI刷新列表
            EaseEvent event = EaseEvent.create(EaseConstant.CONTACT_UPDATE, EaseEvent.TYPE.CONTACT);
            event.message = userInfos.keySet().toString();

            //发送联系人更新事件
            LiveDataBus.get().with(EaseConstant.CONTACT_UPDATE).postValue(event);
        }
