...
public class ChatView extends ChatLayout{
    ...
    public ChatView(Context context, AttributeSet attrs) {
        ...
        if(attrs != null){
            ...
            if(typedArray.hasValue(R.styleable.chat_arrow_direction)) {
                arrowDirection = typedArray.getString(R.styleable.chat_arrow_direction);
            }
            if(typedArray.hasValue(R.styleable.chat_arrow_up_distance)) {
                arrowUpDistance = typedArray.getDimensionPixelSize(R.styleable.chat_arrow_up_distance,50);
            }
            if (typedArray.hasValue(R.styleable.chat_conner_radius)) {
    ...
    public StateListDrawable getSelectorBackground() {
        ...
        /* 按压时显示的背景 */
        chatShape = new ChatShape(arrowWidth, arrowHeight, isArrowCenter, strokeWidth, arrowDirection, arrowUpDistance, connerRadius, pressStrokeColor, pressFillColor);
