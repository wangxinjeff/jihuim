...
import com.hyphenate.mediapicker.cameralibrary.util.EMAngleUtil;
import com.hyphenate.mediapicker.cameralibrary.util.EMCameraParamUtil;
import com.hyphenate.mediapicker.cameralibrary.util.EMCheckPermission;
import com.hyphenate.mediapicker.cameralibrary.util.EMDeviceUtil;
...
public class EMCameraInterface implements Camera.PreviewCallback {

    private static final String TAG = "CJT";

    private volatile static EMCameraInterface mEMCameraInterface;

    public static void destroyCameraInterface() {
        if (mEMCameraInterface != null) {
            mEMCameraInterface = null;
        }
    }
     ...
     * 视频质量
     */
    private int mediaQuality = JEMCameraView.MEDIA_QUALITY_MIDDLE;


    private SensorManager sensorManager = null;

    //获取CameraInterface单例
    public static synchronized EMCameraInterface getInstance() {
        if (mEMCameraInterface == null) {
            synchronized (EMCameraInterface.class) {
                if (mEMCameraInterface == null) {
                    mEMCameraInterface = new EMCameraInterface();
                }
            }
        }
        return mEMCameraInterface;
    }
    ...
    public void setSwitchView(ImageView mSwitchView, ImageView mFlashLamp) {
        ...
        if (mSwitchView != null) {
            cameraAngle = EMCameraParamUtil.getInstance().getCameraDisplayOrientation(mSwitchView.getContext(),
                    SELECTED_CAMERA);
        }
    }
    ...
    private SensorEventListener sensorEventListener = new SensorEventListener() {
        ...
        public void onSensorChanged(SensorEvent event) {
            ...
            angle = EMAngleUtil.getSensorAngle(values[0], values[1]);
            rotationAnimation();
        }
    ...
    public void setZoom(float zoom, int type) {
        ...
        switch (type) {
            ...
            case TYPE_CAPTURE:
                ...
                EMLogUtil.i("setZoom = " + nowScaleRate);
                break;
        }
    ...
    void doOpenCamera(CameraOpenOverCallback callback) {
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.M) {
            if (!EMCheckPermission.isCameraUseable(SELECTED_CAMERA) && this.EMErrorListener != null) {
                this.EMErrorListener.onError();
                return;
            }
        }
        if (mCamera == null) {
...
//        mCamera = Camera.open();
    ...
    public void doStartPreview(SurfaceHolder holder, float screenProp) {
        if (isPreviewing) {
            EMLogUtil.i("doStartPreview isPreviewing");
        }
        ...
        if (mCamera != null) {
            try {
                mParams = mCamera.getParameters();
                Camera.Size previewSize = EMCameraParamUtil.getInstance().getPreviewSize(mParams
                        .getSupportedPreviewSizes(), 1000, screenProp);
                Camera.Size pictureSize = EMCameraParamUtil.getInstance().getPictureSize(mParams
                ...
                if (EMCameraParamUtil.getInstance().isSupportedFocusMode(
                        mParams.getSupportedFocusModes(),
                        Camera.Parameters.FOCUS_MODE_AUTO)) {
                    mParams.setFocusMode(Camera.Parameters.FOCUS_MODE_AUTO);
                }
                if (EMCameraParamUtil.getInstance().isSupportedPictureFormats(mParams.getSupportedPictureFormats(),
                        ImageFormat.JPEG)) {
...
//
    ...
    public void startRecord(Surface surface, float screenProp, ErrorCallback callback) {
        ...
        if (mParams.getSupportedVideoSizes() == null) {
            videoSize = EMCameraParamUtil.getInstance().getPreviewSize(mParams.getSupportedPreviewSizes(), 600,
                    screenProp);
        } else {
            videoSize = EMCameraParamUtil.getInstance().getPreviewSize(mParams.getSupportedVideoSizes(), 600,
                    screenProp);
        }
...
//        }
        ...
        if (EMDeviceUtil.isHuaWeiRongyao()) {
        ...
        File file = new File(EMMediaPathUtil.getInstance().getImagePath(), videoFileName);
    ...
    public void stopRecord(boolean isShort, StopRecordCallback callback) {
        ...
        if (mediaRecorder != null) {
            ...
            if (isShort) {
                if (EMFileUtil.deleteFile(videoFileAbsPath)) {
    ...
    private static Rect calculateTapArea(float x, float y, float coefficient, Context context) {
        ...
        int centerX = (int) (x / EMScreenUtils.getScreenWidth(context) * 2000 - 1000);
        int centerY = (int) (y / EMScreenUtils.getScreenHeight(context) * 2000 - 1000);
    ...
    void setErrorLinsenter(EMErrorListener errorLisenter) {
        this.EMErrorListener = errorLisenter;
    }
    ...
    public boolean isMirror() {
        return isMirror && SELECTED_CAMERA == CAMERA_FRONT_POSITION;
    }
