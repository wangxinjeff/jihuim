...
import com.hyphenate.mediapicker.cameralibrary.listener.EMReturnListener;
...
public class EMCaptureLayout extends FrameLayout {

    private EMCaptureListener captureLisenter;    //拍照按钮监听
    private EMTypeListener typeLisenter;          //拍照或录制后接结果按钮监听
    private EMReturnListener EMReturnListener;      //退出按钮监听
    private EMClickListener leftEMClickListener;    //左边按钮监听
    private EMClickListener rightEMClickListener;   //右边按钮监听

    public void setTypeLisenter(EMTypeListener typeLisenter) {
        this.typeLisenter = typeLisenter;
    }

    public void setCaptureLisenter(EMCaptureListener captureLisenter) {
        this.captureLisenter = captureLisenter;
    }

    public void setReturnLisenter(EMReturnListener EMReturnListener) {
        this.EMReturnListener = EMReturnListener;
    }

    private EMCaptureButton btn_capture;      //拍照按钮
    private EMTypeButton btn_confirm;         //确认按钮
    private EMTypeButton btn_cancel;          //取消按钮
    private EMReturnButton btn_return;        //返回按钮
    ...
    public EMCaptureLayout(Context context) {
        this(context, null);
    }

    public EMCaptureLayout(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public EMCaptureLayout(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    private void initView() {
        ...
        //取消按钮
        btn_cancel = new EMTypeButton(getContext(), EMTypeButton.TYPE_CANCEL, button_size);
...
//                resetCaptureLayout();
        ...
        //确认按钮
        btn_confirm = new EMTypeButton(getContext(), EMTypeButton.TYPE_CONFIRM, button_size);
...
//                resetCaptureLayout();
        ...
        //返回按钮
        btn_return = new EMReturnButton(getContext(), (int) (button_size / 2.5f));
    ...
    public void setLeftEMClickListener(EMClickListener leftEMClickListener) {
        this.leftEMClickListener = leftEMClickListener;
    }

    public void setRightEMClickListener(EMClickListener rightEMClickListener) {
        this.rightEMClickListener = rightEMClickListener;
    }
}
