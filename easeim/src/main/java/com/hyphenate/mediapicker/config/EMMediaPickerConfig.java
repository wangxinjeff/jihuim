 ...
 * @date : 2019/4/16 9:51
 */
public class EMMediaPickerConfig {
     ...
     * CAMERA：直接跳转照相机
     */
    private EMMediaPickerEnum EMMediaPickerEnum;

    public EMMediaPickerConfig() {
    ...
    public int getCameraMediaType() {
        if (getMaxImageSelectable() == 0 && getMaxVideoSelectable() > 0) {
            return JEMCameraView.BUTTON_STATE_ONLY_RECORDER;
        } else if (getMaxImageSelectable() > 0 && getMaxVideoSelectable() == 0) {
            return JEMCameraView.BUTTON_STATE_ONLY_CAPTURE;
        }
        return JEMCameraView.BUTTON_STATE_BOTH;
    }

    public Set getPhotoPickerMediaType() {
        if (getMaxImageSelectable() == 0 && getMaxVideoSelectable() > 0) {
            return EMMimeType.ofVideo();
        } else if (getMaxImageSelectable() > 0 && getMaxVideoSelectable() == 0) {
            return EMMimeType.ofImage();
        }
        return EMMimeType.ofAll();
    }


    public EMImageEngine getEMImageEngine() {
        return EMImageEngine;
    }

    public void setEMImageEngine(EMImageEngine EMImageEngine) {
        this.EMImageEngine = EMImageEngine;
    }


    public EMMediaPickerEnum getEMMediaPickerEnum() {
        return EMMediaPickerEnum;
    }

    public void setEMMediaPickerEnum(EMMediaPickerEnum EMMediaPickerEnum) {
        this.EMMediaPickerEnum = EMMediaPickerEnum;
    }
