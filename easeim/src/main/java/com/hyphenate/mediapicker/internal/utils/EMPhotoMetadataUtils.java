...
public final class EMPhotoMetadataUtils {
    private static final String TAG = EMPhotoMetadataUtils.class.getSimpleName();
    private static final int MAX_WIDTH = 1600;
    private static final String SCHEME_CONTENT = "content";

    private EMPhotoMetadataUtils() {
    ...
    public static Point getBitmapSize(Uri uri, Activity activity) {
        ...
        int w = imageSize.x;
        int h = imageSize.y;
        if (EMPhotoMetadataUtils.shouldRotate(resolver, uri)) {
            w = imageSize.y;
            h = imageSize.x;
        }
    ...
    public static EMIncapableCause isAcceptable(Context context, EMItem item) {
        if (!isSelectableType(context, item)) {
            return new EMIncapableCause(context.getString(R.string.em_error_file_type));
        }

        if (EMSelectionSpec.getInstance().filters != null) {
            for (EMFilter filter : EMSelectionSpec.getInstance().filters) {
                EMIncapableCause EMIncapableCause = filter.filter(context, item);
                if (EMIncapableCause != null) {
                    return EMIncapableCause;
                }
            }
        }
        return null;
    ...
    private static boolean isSelectableType(Context context, EMItem item) {
        if (context == null) {
            return false;
        }

        ContentResolver resolver = context.getContentResolver();
        for (EMMimeType type : EMSelectionSpec.getInstance().EMMimeTypeSet) {
    ...
    private static boolean shouldRotate(ContentResolver resolver, Uri uri) {
        ExifInterface exif;
        try {
            exif = EMExifInterfaceCompat.newInstance(getPath(resolver, uri));
        } catch (IOException e) {
