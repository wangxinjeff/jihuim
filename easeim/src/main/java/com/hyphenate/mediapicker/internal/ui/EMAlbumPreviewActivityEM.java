...
public class EMAlbumPreviewActivityEM extends EMBasePreviewActivity implements
        EMAlbumMediaCollection.AlbumMediaCallbacks {

    public static final String EXTRA_ALBUM = "extra_album";
    public static final String EXTRA_ITEM = "extra_item";

    private EMAlbumMediaCollection mCollection = new EMAlbumMediaCollection();
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        EMAlbum album = getIntent().getParcelableExtra(EXTRA_ALBUM);
        mCollection.load(album);

        EMItem item = getIntent().getParcelableExtra(EXTRA_ITEM);
        if (mSpec.countable) {
    ...
    public void onAlbumMediaLoad(Cursor cursor) {
        List<EMItem> items = new ArrayList<>();
        while (cursor.moveToNext()) {
            items.add(EMItem.valueOf(cursor));
        }
//        cursor.close();
        ...
        if (!mIsAlreadySetPosition) {
            ...
            EMItem selected = getIntent().getParcelableExtra(EXTRA_ITEM);
