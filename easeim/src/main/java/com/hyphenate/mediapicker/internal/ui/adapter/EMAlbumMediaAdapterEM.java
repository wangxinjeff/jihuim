...
import com.hyphenate.mediapicker.internal.ui.widget.EMMediaGrid;

public class EMAlbumMediaAdapterEM extends
        EMRecyclerViewCursorAdapter<RecyclerView.ViewHolder> implements
        EMMediaGrid.OnMediaGridClickListener {

    private static final int VIEW_TYPE_CAPTURE = 0x01;
    private static final int VIEW_TYPE_MEDIA = 0x02;
    private final EMSelectedItemCollection mSelectedCollection;
    private final Drawable mPlaceholder;
    private EMSelectionSpec mSelectionSpec;
    private CheckStateListener mCheckStateListener;
    private OnMediaClickListener mOnMediaClickListener;
    private RecyclerView mRecyclerView;
    private int mImageResize;

    public EMAlbumMediaAdapterEM(Context context, EMSelectedItemCollection selectedCollection, RecyclerView recyclerView) {
        super(null);
        mSelectionSpec = EMSelectionSpec.getInstance();
        mSelectedCollection = selectedCollection;

        TypedArray ta = context.getTheme().obtainStyledAttributes(new int[]{R.attr.emitem_placeholder});
    ...
    protected void onBindViewHolder(final RecyclerView.ViewHolder holder, Cursor cursor) {
        if (holder instanceof CaptureViewHolder) {
            ...
            TypedArray ta = holder.itemView.getContext().getTheme().obtainStyledAttributes(
                    new int[]{R.attr.emcapture_textColor});
            ...
            final EMItem item = EMItem.valueOf(cursor);
            mediaViewHolder.mMediaGrid.preBindMedia(new EMMediaGrid.PreBindInfo(
    ...
    private void setCheckStatus(EMItem item, EMMediaGrid mediaGrid) {
        if (mSelectionSpec.countable) {
            ...
            if (checkedNum > 0) {
                ...
                if (mSelectedCollection.maxSelectableReached()) {
                    ...
                    mediaGrid.setCheckedNum(EMCheckView.UNCHECKED);
                } else {
                    mediaGrid.setCheckEnabled(true);
                    mediaGrid.setCheckedNum(checkedNum);
                }
    ...
    @Override
    public void onThumbnailClicked(ImageView thumbnail, EMItem item, RecyclerView.ViewHolder holder) {
    ...
    @Override
    public void onCheckViewClicked(EMCheckView checkView, EMItem item, RecyclerView.ViewHolder holder) {
        updateSelectedItem(item, holder);
    }

    private void updateSelectedItem(EMItem item, RecyclerView.ViewHolder holder) {
        if (mSelectionSpec.countable) {
            int checkedNum = mSelectedCollection.checkedNumOf(item);
            if (checkedNum == EMCheckView.UNCHECKED) {
    ...
    public int getItemViewType(int position, Cursor cursor) {
        return EMItem.valueOf(cursor).isCapture() ? VIEW_TYPE_CAPTURE : VIEW_TYPE_MEDIA;
    }

    private boolean assertAddSelection(Context context, EMItem item) {
        EMIncapableCause cause = mSelectedCollection.isAcceptable(item);
        EMIncapableCause.handleCause(context, cause);
        return cause == null;
    }
        ...
        for (int i = first; i <= last; i++) {
            ...
            if (holder instanceof MediaViewHolder) {
                if (cursor.moveToPosition(i)) {
                    setCheckStatus(EMItem.valueOf(cursor), ((MediaViewHolder) holder).mMediaGrid);
                }
            }
        }
    }
    ...
    public interface OnMediaClickListener {
        void onMediaClick(EMAlbum album, EMItem item, int adapterPosition);
    }
    ...
    private static class MediaViewHolder extends RecyclerView.ViewHolder {
        ...
        private EMMediaGrid mMediaGrid;

        MediaViewHolder(View itemView) {
            super(itemView);
            mMediaGrid = (EMMediaGrid) itemView;
        }
    }
