 ...
 */
package com.hyphenate.mediapicker.internal.model;
...
public class EMSelectedItemCollection {
    ...
    private Set<EMItem> mItems;
    private int mCollectionType = COLLECTION_UNDEFINED;

    public EMSelectedItemCollection(Context context) {
        mContext = context;
    }
    ...
    public void onCreate(Bundle bundle) {
        if (bundle == null) {
            ...
            List<EMItem> saved = bundle.getParcelableArrayList(STATE_SELECTION);
            mItems = new LinkedHashSet<>(saved);
            mCollectionType = bundle.getInt(STATE_COLLECTION_TYPE, COLLECTION_UNDEFINED);
        }
    }

    public void setDefaultSelection(List<EMItem> uris) {
        mItems.addAll(uris);
    }
    ...
    public boolean add(EMItem item) {
    ...
    public boolean remove(EMItem item) {
    ...
    public void overwrite(ArrayList<EMItem> items, int collectionType) {
        if (items.size() == 0) {
            mCollectionType = COLLECTION_UNDEFINED;
        } else {
    ...
    public List<EMItem> asList() {
        return new ArrayList<>(mItems);
    }

    public List<Uri> asListOfUri() {
        List<Uri> uris = new ArrayList<>();
        for (EMItem item : mItems) {
    ...
    public List<String> asListOfString() {
        ...
        for (EMItem item : mItems) {
            paths.add(EMPathUtils.getPath(mContext, item.getContentUri()));
        }
        return paths;
    }
    ...
    public boolean isSelected(EMItem item) {
        return mItems.contains(item);
    }

    public EMIncapableCause isAcceptable(EMItem item) {
        if (maxSelectableReached()) {
            ...
            return new EMIncapableCause(cause);
        } else if (typeConflict(item)) {
            return new EMIncapableCause(mContext.getString(R.string.em_error_type_conflict));
        }

        return EMPhotoMetadataUtils.isAcceptable(mContext, item);
    }
     ...
     * while {@link EMSelectionSpec#mediaTypeExclusive} is set to false.
     */
    public boolean typeConflict(EMItem item) {
        return EMSelectionSpec.getInstance().mediaTypeExclusive
    ...
    public int checkedNumOf(EMItem item) {
        int index = new ArrayList<>(mItems).indexOf(item);
        return index == -1 ? EMCheckView.UNCHECKED : index + 1;
    }
}
